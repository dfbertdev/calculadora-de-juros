<template>
    <div class="simulador-container">
      ...
      <div class="form-container">
        ...
        <!-- Linha 3: Mês de Início e Data de Nascimento com Tipo -->
        <div class="row">
          <div class="form-group">
        ...
        <table class="tabela-detalhes">
          ...
          <tbody>
            <tr v-for="(item, index) in detalhes" :key="index">
              ...
              <td>{{ formatCurrency(item.totalAportadoSoFar) }}</td>
  ...
  export default {
    ...
    data() {
      return {
        ...
        exibirAlerta: false,
        simulacoesSalvas: [] // Array para simulações salvas no localStorage
      }
    },
    computed: {
      isFirstSegment() {
    ...
    methods: {
      ...
      getAgeAt(mesIndex) {
        if (!this.dataNascimento) return '-'
        const simDate = new Date(this.mesInicio + "-01")
      ...
      adicionarSegmentoInterno({ valorInicialSegmento, valorMensal, taxaDecimal, taxaExibicao, periodo }) {
        ...
        const segInvestidoInicial = (this.segmentos.length === 0)
          ? valorInicialSegmento
          : this.segmentos[this.segmentos.length - 1].montanteFinal
        const segMontanteInicial = (this.segmentos.length === 0)
          ? valorInicialSegmento
          : this.segmentos[this.segmentos.length - 1].montanteFinal
  
        const totalInvestidoFinal = last.totalInvestido
        const totalAcumuladoFinal = last.totalAcumulado
  
        const investidoNoSegmento = totalInvestidoFinal - segInvestidoInicial
        const jurosNoSegmento = (totalAcumuladoFinal - segMontanteInicial) - investidoNoSegmento
        ...
        const segObj = {
          ...
          valorMensal,
          taxaExibicao,
          investido: investidoNoSegmento,
          juros: jurosNoSegmento,
          montanteFinal: totalAcumuladoFinal
        }
        this.segmentos.push(segObj)
  
        this.resultado.totalFinal = totalAcumuladoFinal
        this.resultado.totalInvestido = totalInvestidoFinal
        this.resultado.totalJuros = totalAcumuladoFinal - totalInvestidoFinal
      ...
      // Função para salvar a simulação atual no localStorage
      salvarSimulacao() {
        const sim = {
          id: Date.now(),
